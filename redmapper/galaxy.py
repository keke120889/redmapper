...
class GalaxyCatalog(Catalog):
    ...
    def from_galfile(cls, filename, nside=None, hpix=None, border=0.0):
        ...
        try:
            ...
            # this is not a pixelated file.  Just a fits file, which we can read above!
            # not sure if this is the right calling
            super(GalaxyCatalog, self).from_fits_file(filename)
            return
        ...
        nside_tab = tab['NSIDE']
        if (nside > nside_tab):
            raise ValueError("Requested nside (%d) must not be larger than table nside (%d)." % (nside, nside_tab))
        ...
        if (hpix is None):
            ...
            if (border > 0.0):
                ...
                step = nside_tab/nside
                boundaries = hp.boundaries(nside, hpix, step = step)

                inhpix = tab[0]['HPIX'][indices]
                
                for i in xrange(boundaries.shape[1]):
                    pixint=hp.query_disc(nside_tab,boundaries[:,i],border*np.pi/180.,inclusive=True,fact=8)
